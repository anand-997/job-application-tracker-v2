...
| 🔗 | **Linked data folder** — during onboarding, pick a folder on your device (File System Access API); JobTracker reads/creates `jobtracker-backup.json` there and **auto-saves every change** to it. On return the board shows instantly and re-links in **one click**. Chromium-only; Firefox/Safari fall back to manual Export/Import |
